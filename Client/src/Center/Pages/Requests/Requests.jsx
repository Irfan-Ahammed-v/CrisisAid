...
const Requests = () => {
  ...
  const filteredRequests = requests.filter((request) => {
    const matchesPriority =
      filterPriority === "all" || request.request_priority === filterPriority;
    const matchesStatus =
      filterStatus === "all" || request.request_status === filterStatus;
    const matchesSearch =
      request.camp_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleSubmitReply = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:5000/center/updateRequest/${selectedRequest._id}`,
        {
          status,
          reply: replyMessage,
          estimatedVolunteers:
            status === "accepted" ? estimatedVolunteers : undefined,
        },
      );
  ...
  const handleAssignVolunteers = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/center/assignVolunteers/${selectedRequest._id}`,
        {
      ...
      showNotification(
        "success",
        `Assigned ${selectedVolunteers.length} volunteer(s) to this request`,
      );
    ...
    <>
      ...
      <div className="min-h-screen bg-[#0d1117] text-slate-300">
        ...
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(rgba(232,162,62,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(232,162,62,0.025) 1px,transparent 1px)",
        ...
        <div className="relative z-10 max-w-[1380px] mx-auto px-6 py-10">
          ...
          {/* ── Toolbar ── */}
          <div
            className="fu bg-[#161b22] border border-[#30363d] rounded-2xl p-5 mb-5 shadow-xl"
          ...
          >
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-[#21262d]">
              ...
              <span className="text-lg font-bold text-slate-300 font-mono">
                ...
                <span className="text-xs text-slate-600 font-normal ml-1.5">
                  Active
                </span>
              </span>
            </div>
          ...
          <div className="fu space-y-4" style={{ animationDelay: "100ms" }}>
            {filteredRequests.length > 0 ? (
              filteredRequests.map((request, i) => {
                const pri =
                  PRI_BADGE[request.request_priority] || PRI_BADGE.low;
                ...
                const initials = request.camp_name
                  .split(" ")
                  .map((w) => w[0])
                  .join("")
                  .slice(0, 2)
                ...
                return (
                  <div
                    ...
                    style={{
                      animation: `fadeUp 0.4s cubic-bezier(0.16,1,0.3,1) ${i * 40}ms both`,
                    }}
                  >
                    {/* Priority accent bar */}
                    ...
                    <div className="flex items-start gap-4 p-5 border-b border-[#21262d]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mt-2.5">
                          <PriBadge
                            p={request.request_priority}
                            isAi={request.isAiGenerated}
                          />
      ...
      {assignModalOpen && selectedRequest && (
        ...
        >
          <div
            className="bg-[#161b22] border border-[#30363d] rounded-2xl w-full max-w-3xl max-h-[88vh] overflow-y-auto shadow-2xl"
              ...
              <div className="bg-[#0d1117] border border-[#21262d] rounded-xl p-4 mb-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {selectedRequest.items.slice(0, 4).map((item, idx) => (
                    <span
                      key={idx}
                      className="px-2 py-0.5 bg-[#161b22] border border-[#30363d] rounded-lg text-xs text-slate-400 font-mono"
                    >
                ...
                {volunteers.map((volunteer) => {
                  ...
                  return (
                    <button
                      key={volunteer._id}
                      onClick={() =>
                        isAvailable && toggleVolunteerSelection(volunteer._id)
                      }
      ...
      {replyModalOpen && selectedRequest && (
        ...
        >
          <div
            className="bg-[#161b22] border border-[#30363d] rounded-2xl w-full max-w-xl shadow-2xl"
            onClick={(e) => e.stopPropagation()}
            style={{ animation: "scaleIn 0.22s cubic-bezier(0.16,1,0.3,1)" }}
          >
            <div
              className={`h-0.5 w-full rounded-t-2xl ${replyAction === "accept" ? "bg-emerald-500" : "bg-red-500"}`}
            />
                  ...
                  <div className="mt-4 mb-2">
                    ...
                    <div className="relative flex items-center">
                      ...
                      <input
                        ...
                        onChange={(e) => setEstimatedVolunteers(e.target.value)}
                      />
                    </div>
                  </div>
                )}
