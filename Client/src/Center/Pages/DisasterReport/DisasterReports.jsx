...
const DisasterReports = () => {
  ...
  const handleStatusChange = (disasterId, newStatus) => {
    ...
    axios
      ...
      .then(() => {
        ...
        if (selectedDisaster?._id === disasterId) {
          setSelectedDisaster((prev) => ({ ...prev, disaster_status: newStatus }));
        }
      })
      .catch((err) => {
  ...
  function StatBadge({ s }) {
    const statusKey =
      s === null || s === "null" || s === undefined ? "pending" : s;
    const c = STAT_BADGE[statusKey] || STAT_BADGE.pending;
    return (
      <span
  ...
  return (
    <>
          ...
          <div className="fu fu3 grid grid-cols-1 lg:grid-cols-2 gap-5">
            {filteredDisasters.length > 0 ? (
              filteredDisasters.map((disaster, i) => {
                ...
                const typeName =
                  disaster.disaster_type?.disaster_type_name || "Unknown Type";
                const placeName =
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <div className="flex gap-2 mb-3">
                        ...
                        {isPending && (
                          <div className="relative flex items-center justify-center px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider bg-amber-400/10 text-amber-500 border border-amber-400/20 animate-pulse">
                            Needs Review
                          </div>
                        )}
                      </div>

                      <div className="pt-3 border-t border-[#21262d] flex gap-2">
                        {isPending ? (
                          <>
                            <button
                              onClick={() =>
                                handleStatusChange(disaster._id, "active")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleStatusChange(disaster._id, "rejected")
                              }
                          ...
                          <button
                            onClick={() =>
                              handleStatusChange(disaster._id, "resolved")
                            }
      ...
      {detailModalOpen && selectedDisaster && (
        ...
        >
          <div
            className="bg-[#161b22] border border-[#30363d] rounded-2xl w-full max-w-2xl max-h-[88vh] overflow-y-auto shadow-2xl"
            ...
            <div className="flex items-start gap-4 p-6 border-b border-[#21262d]">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 font-semibold mt-0.5">
                  {selectedDisaster.place_id?.place_name}
                </p>
              </div>
              <button
                onClick={() => setDetailModalOpen(false)}
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[#21262d]">
              ...
              {(selectedDisaster.disaster_status === null ||
                ...
                <>
                  <button
                    onClick={() =>
                      handleStatusChange(selectedDisaster._id, "active")
                    }
                  ...
                  <button
                    onClick={() =>
                      handleStatusChange(selectedDisaster._id, "rejected")
                    }
              ...
              {selectedDisaster.disaster_status === "active" && (
                <button
                  onClick={() =>
                    handleStatusChange(selectedDisaster._id, "resolved")
                  }
